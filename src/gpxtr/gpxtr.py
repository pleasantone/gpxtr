...
class GPXCalculator:
    ...
    def __init__(self, gpx: GPX, imperial=True, speed=None, departure=None, waypoint_delays=None) -> None:
        self.gpx = gpx
        if speed:
            self.speed =  speed / KM_TO_MILES if imperial else speed
        else:
    ...
    def travel_time(self, dist) -> timedelta:
...
def main() -> None:
    ...
    for handle in args.input:
        with handle as stream:
            try:
                table = GPXCalculator(gpxpy.parse(stream), speed=args.speed, departure=args.departure)
