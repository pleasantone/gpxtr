...
class GPXTableCalculator:
    ...
    def print_waypoints(self, sort: str='', out: Optional[io.TextIOWrapper]=None) -> None:
        ...
        def _wpe() -> str:
            return OUT_FMT.format(
                ...
                point.symbol or '',
                f' (+{str(self.symbol_delay(point))[:-3]})' if not first_point and self.symbol_delay(point) else '')
    ...
    #            f'START: {track.name}', 'START'))
        ...
        if self.gpx.waypoints and self.gpx.tracks:
            ...
            first_point = True
            for point in self.geodata_nearest(self.geodata_points(self.gpx.waypoints),
                                              self.geodata_tracks(self.gpx.tracks), sort=sort).itertuples():
                departure = start_time + self.travel_time(point.track_distance) if start_time else None
    ...
    def print_routes(self, out: Optional[io.TextIOWrapper]=None) -> None:
        ...
        def _rpe() -> str:
            return OUT_FMT.format(
                ...
                (point.name or '').replace('\n', ' '),
                f'{self.format_long_length(dist - last_gas)}/{self.format_long_length(dist)}' if self.is_gas(point) or point is route.points[-1]
        ...
        for route in self.gpx.routes:
            ...
            for point in route.points:
                ...
                dist += distance(previous[0], previous[1], None, current[0], current[1], None)
    ...
    @staticmethod
    def waypoint_from_point(point, name: str, symbol: Optional[str]=None) -> GPXWaypoint:
    ...
    @staticmethod
    def geodata_nearest(points: gpd.GeoDataFrame, tracks: gpd.GeoDataFrame, sort: str='') -> pd.DataFrame:
